...
const DecideItQuadrant = ({ navigation }: any) => {
    ...
    const getDisplayNotes = async (): Promise<string[]> => {
        try {
            const displayNotes = await AsyncStorage.getItem('decideit');
    ...
    const handleEditNote = (note: string, index: number) => {
        navigation.navigate('EditDecideIt', { note, index });
    };

    const deleteDecideNoteEntry = async (note: string, index: number) => {
        try {
            const existingNoteEntries = await AsyncStorage.getItem('decideit');
            ...
            await AsyncStorage.setItem('decideit', JSON.stringify(noteEntriesArray));
            ...
            <View style={styles.quadrantContainer}>
                <View style={styles.quadrantHeaderContainer}>
                    <TouchableOpacity style={styles.quadrantHeader} onPress={() => navigation.navigate('Matrix')}>
                        ...
                        <Text style={styles.quadrantText}>Decide It</Text>
                    </TouchableOpacity>
                </View>
                ...
                <View style={styles.addButtonContainer}>
                    <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('AddDecideIt')} >
...
export default DecideItQuadrant;
