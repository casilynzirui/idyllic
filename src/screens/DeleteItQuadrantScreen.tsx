...
const DeleteItQuadrant = ({ navigation }: any) => {
    ...
    const getDisplayNotes = async (): Promise<string[]> => {
        try {
            const displayNotes = await AsyncStorage.getItem('deleteit');
    ...
    const handleEditNote = (note: string, index: number) => {
        navigation.navigate('EditDeleteIt', { note, index });
    };

    const deleteDeleteNoteEntry = async (note: string, index: number) => {
        try {
            ...
            <View style={styles.quadrantContainer}>
                <View style={styles.quadrantHeaderContainer}>
                    <TouchableOpacity style={styles.quadrantHeader} onPress={() => navigation.navigate('Matrix')}>
                        ...
                        <Text style={styles.quadrantText}>Delete It</Text>
                    </TouchableOpacity>
                </View>
                        ...
                        {displayNotes.map((note, index) => (
                            <TouchableOpacity key={index} style={styles.displayNoteContainer} onPress={() => handleEditNote(note, index)}>
                                <View style={styles.displayNoteContainer2}>
                                    <TouchableOpacity style={styles.checkBox} onPress={() => deleteDeleteNoteEntry(note, index)} />
                ...
                <View style={styles.addButtonContainer}>
                    <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('AddDeleteIt')} >
...
export default DeleteItQuadrant;
