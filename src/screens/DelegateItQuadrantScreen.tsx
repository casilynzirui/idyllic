...
const DelegateItQuadrant = ({ navigation }: any) => {
    ...
    const getDisplayNotes = async (): Promise<string[]> => {
        try {
            const displayNotes = await AsyncStorage.getItem('delegateit');
    ...
    const handleEditNote = (note: string, index: number) => {
        navigation.navigate('EditDelegateIt', { note, index });
    };

    const deleteDelegateNoteEntry = async (note: string, index: number) => {
        try {
            ...
            <View style={styles.quadrantContainer}>
                <View style={styles.quadrantHeaderContainer}>
                    <TouchableOpacity style={styles.quadrantHeader} onPress={() => navigation.navigate('Matrix')}>
                        ...
                        <Text style={styles.quadrantText}>Delegate It</Text>
                    </TouchableOpacity>
                </View>
                        ...
                        {displayNotes.map((note, index) => (
                            <TouchableOpacity key={index} style={styles.displayNoteContainer} onPress={() => handleEditNote(note, index)}>
                                <View style={styles.displayNoteContainer2}>
                                    <TouchableOpacity style={styles.checkBox} onPress={() => deleteDelegateNoteEntry(note, index)} />
                ...
                <View style={styles.addButtonContainer}>
                    <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('AddDelegateIt')} >
...
export default DelegateItQuadrant;
